...
struct sockaddr_in server_TIS,server_C,server_E;
...
{   
    ...
    server_E.sin_addr.s_addr =inet_addr("127.0.0.1");
    server_E.sin_port=htons(E_PORT);
}
...
int check_for_other_malls_slots()
{
    ...
    cli_len=sizeof(server_E);
    sendto(sfd_udp,"query\0",strlen("query")+1,0,
        (struct sockaddr *) &server_E,cli_len);
...
main()
{
    ...
    pthread_t C_pid,E_pid;
    pthread_create(&C_pid,NULL,handle_mall_connection,(struct sockaddr_in *)&(server_C));
    pthread_create(&E_pid,NULL,handle_mall_connection,(struct sockaddr_in *)&(server_E));
